...
def load_tracks_np(track_path, frame_start, frame_end, also_first=False):
...
def load_video(video_path, frame_start=0, frame_ends=49):
    ...
    frames = []
    for fid in range(frame_start, frame_ends):
...
class Visualizer():
    def __init__(self, cond_option="id+color", run_abl="", h=480, w=720, c=3, num_frames=49):
    ...
    def get_cond_maps(self, track_path, rle_path, frames, use_color=True, frame_start=0, is_real_video=True,
        ...
        # 0. convert the frames from PIL to numpy
        frames = np.stack([np.array(f) for f in frames]) if frames is not None else frames
        ...
        if "color" in self.cond_option.split("+"):
            color_maps = np.zeros_like(id_maps)
            # print("use_color: ", use_color)
            if use_color:
                if is_real_video:
                    mask = (id_maps[...,0] > 0)[...,None]
                else:
                    mask = (id_maps[...,0] > 20)[...,None]
                color_maps = frames*mask
        else:
            color_maps = None

        # 3. add maps to cond_maps_dict
        id_maps = [Image.fromarray(f) for f in id_maps]
        color_maps = [Image.fromarray(f) for f in color_maps]
        return [id_maps, color_maps]
