...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def switch_pipe_to_training_mode(
        ...
        if trainable_models == "fuse_linear" or trainable_models == "patch_embedding":
            pipe.freeze_except([])
        else:
        ...
        if trainable_models == "fuse_linear":
            pipe.dit.fuse_linear.weight.requires_grad_(True)
            pipe.dit.fuse_linear.bias.requires_grad_(True)
        if trainable_models == "patch_embedding":
            pipe.dit.patch_embedding.weight.requires_grad = True
            pipe.dit.patch_embedding.bias.requires_grad = True
...
class ModelLogger:
    ...
    def on_step_end(self, accelerator, model, save_steps=None):
        self.num_steps += 1
        if save_steps is not None and (self.num_steps - 1) % save_steps == 0:
            self.save_model(accelerator, model, f"step-{self.num_steps}.safetensors")
            if accelerator.is_main_process:
                one_validation_is_enough = True
                if (self.num_steps - 1) % (2 * save_steps) == 0:
                    one_validation_is_enough = False
                save_prefix = os.path.join(self.output_path, f"vid-{self.num_steps}-")
                model.valid_infer(save_prefix, one_validation_is_enough=one_validation_is_enough)
    ...
    def on_epoch_end(self, accelerator, model, epoch_id):
        accelerator.wait_for_everyone()
        if accelerator.is_main_process:
            print(f"finish epoch {epoch_id}")
            # if epoch_id !=0 and (epoch_id+1) % 10 == 0:
            if True:
...
def launch_training_task(
    ...
    optimizer = torch.optim.AdamW(model.trainable_modules(), lr=learning_rate, weight_decay=weight_decay,  betas=(0.9, 0.95), eps=1e-8)
    scheduler = torch.optim.lr_scheduler.ConstantLR(optimizer)
    dataloader = torch.utils.data.DataLoader(dataset, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers)

    log_with = "tensorboard"
    project_config = ProjectConfiguration(
        project_dir=model_logger.output_path,  # 训练输出根目录
        logging_dir=os.path.join(model_logger.output_path, "logs")
    )
    ...
    accelerator.init_trackers("exp1")

    model, optimizer, dataloader, scheduler = accelerator.prepare(model, optimizer, dataloader, scheduler)
    
    # # add warm up
    # num_update_steps_per_epoch = (len(dataloader) + gradient_accumulation_steps - 1) // gradient_accumulation_steps
    # max_train_steps = num_epochs * num_update_steps_per_epoch
    # warmup_steps = max(500, int(0.05 * max_train_steps))
    ...
    total_batch_size = accelerator.num_processes * gradient_accumulation_steps
    max_train_steps = int(num_epochs * len(dataset) / total_batch_size)
    if accelerator.is_main_process:
...
def wan_parser():
    ...
    parser.add_argument("--test_root", type=str, default="", help="Root of test directory.")
    parser.add_argument("--validation_file", type=str, default="", help="Validation file path.")
    ...
    parser.add_argument("--is_random_spatial", action="store_true")
    parser.add_argument("--is_random_temporal", action="store_true")
    parser.add_argument("--is_random_shift", action="store_true")
    return parser
