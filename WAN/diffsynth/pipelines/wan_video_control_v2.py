...
from ..models import ModelManager, load_state_dict, load_dit
...
def _globseg_to_regex(pattern: str):
    # Escape everything first, then replace the escaped '*' with a segment wildcard
    esc = re.escape(pattern)
    esc = esc.replace(r'\*', r'[^.]*')
    return re.compile(r'^' + esc + r'$')

def get_t_mod(timestep, dit, fuse_vae_embedding_in_latents, latents, use_unified_sequence_parallel):
...
class WanVideoPipeline(BasePipeline):
    ...
    def freeze_except(self, model_names):
        # set everything except dit to train model
        for name, model in self.named_children():
            model.eval()
            model.requires_grad_(False)
        self.dit.train()
        
        # set new insert layer to have grad=True
        regexes = [_globseg_to_regex(p) for p in model_names]
        ...
        print("trainable parameters: ", matched_params)
    '''
    ...
    def from_pretrained(
        ...
        dit_config = model_configs.pop(0)
        for model_config in model_configs:
    ...
    def __call__(
        ...
        # print("### inputs_shared")
        # print_info(inputs_shared)
        # print("### inputs_posi")
        # print_info(inputs_posi)
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_image, end_image, height, width):
        if input_image is None or pipe.image_encoder is None or not pipe.dit.require_clip_embedding:
            print("WanVideoUnit_ImageEmbedderCLIP has no use")
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        image = pipe.preprocess_image(input_image.resize((width, height))).to(pipe.device)
        clip_context = pipe.image_encoder.encode_image([image])
...
class WanVideoUnit_ImageEmbedderVAE(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
        if input_image is None or not pipe.dit.require_vae_embedding:
            print("pipe.dit.require_vae_embedding: ", pipe.dit.require_vae_embedding)
            print("skip image embedderVAE")
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        image = pipe.preprocess_image(input_image.resize((width, height))).to(pipe.device)
        msk = torch.ones(1, num_frames, height//8, width//8, device=pipe.device)
        ...
        print("start to process image embeds")
        print("vae_input shape: ", vae_input.shape)
        y = pipe.vae.encode([vae_input.to(dtype=pipe.torch_dtype, device=pipe.device)], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)[0]
        y = y.to(dtype=pipe.torch_dtype, device=pipe.device)
        print("y shape: ", y.shape)
        print("mask shape: ", msk.shape)
        print("end process image embeds")
        y = torch.concat([msk, y])
        y = y.unsqueeze(0)
        y = y.to(dtype=pipe.torch_dtype, device=pipe.device)
        return {"y": y}
...
class WanVideoUnit_CrossControl(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("control_video", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, control_video, tiled, tile_size, tile_stride):
        if control_video is None:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        control_video = pipe.preprocess_video(control_video)
...
class WanVideoUnit_SpeedControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, motion_bucket_id):
        if motion_bucket_id is None:
            print("WanVideoUnit_SpeedControl has no use")
            return {}
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # Timestep
    t, t_mod = get_t_mod(timestep, dit, fuse_vae_embedding_in_latents, latents, use_unified_sequence_parallel)
    _, t_mod_ = get_t_mod(0, dit, fuse_vae_embedding_in_latents, latents, use_unified_sequence_parallel)
    t_mod = [t_mod, t_mod_]
    # t_mod = torch.concat([t_mod, t_mod_])
    ...
    if control_latents is not None:
        control_latents = dit.patchify(control_latents)
        f, h, w = control_latents.shape[2:]
        control_latents = rearrange(control_latents, 'b c f h w -> b (f h w) c').contiguous()
        # print("context: ", context.shape)
        # print("control_latents: ", control_latents.shape)
        # context = torch.cat([context, control_latents], dim=1)
        x = torch.cat([x, control_latents], dim=1)
