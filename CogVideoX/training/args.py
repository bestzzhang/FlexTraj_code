...
def _get_dataset_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the seg map video.",
    )
    parser.add_argument(
        "--cond_option",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        choices=["all", "real", "syn"],
    )
    parser.add_argument(
        "--seg_column",
        type=str,
        default=None,
        help="The column of the dataset containing the seg map for each video. Or, the name of the file in `--data_root` folder containing the line-separated seg maps.",
    )
    parser.add_argument(
        "--num_tracking_blocks",
        type=int,
...
def _get_configuration_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--no_train_fuse', 
                        action='store_false', 
                        dest='train_fuse', 
                        help='Disable training of fuse layer')
    parser.add_argument(
        "--allow_tf32",
        action="store_true",
        help=(
