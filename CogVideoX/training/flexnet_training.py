...
from dataset import VideoDatasetUniform  # isort:skip
...
from utils import get_inputs_from_json, infer, rmdir_except_lora
...
from models.cogvideox_flextraj import CogVideoXFlexControlPipeline
from models.cogvideox_flextraj import CogVideoXTransformerFlex
...
def log_validation(
    ...
    one_validation_is_enough = True
    if step % (2 * args.checkpointing_steps) == 0:
        one_validation_is_enough = False

    # Load condition maps
    vis = Visualizer(args.cond_option)
    ...
    count = 0
    for inputs in get_inputs_from_json(args.validation_file, one_validation_is_enough=one_validation_is_enough):   
        print(f"count: {count}")
        concatenated, save_name = infer(inputs, vis, pipe)
        concatenated = concatenated.permute((0, 3, 1, 2))
        frames = [F.to_pil_image(concatenated[i].clamp(0, 1)) for i in range(concatenated.shape[0])]
        count += 1

        for tracker in accelerator.trackers:
            if tracker.name == "wandb":
                filename = os.path.join(args.output_dir, f"ep{step}_{save_name}.mp4")
                os.makedirs(os.path.dirname(filename), exist_ok=True)
                export_to_video(frames, filename, fps=24)
                tracker.log({f"ep{step}_{save_name}": wandb.Video(filename, fps=24, format="mp4")}, step=step)

    torch.cuda.empty_cache()

class CollateFunctionFlexVideo:
    def __init__(self, weight_dtype: torch.dtype, load_tensors: bool, use_buckets: bool=False) -> None:
        self.weight_dtype = weight_dtype
        self.load_tensors = load_tensors
        self.use_buckets = use_buckets

    def __call__(self, data: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        data = data[0] if self.use_buckets else data
        prompts = [x["prompt"] for x in data]
        indexs = [x["index"] for x in data]
        belongs = [x["belong"] for x in data]
        ...
        cond_keys = [x["cond_keys"] for x in data][0]
        cond_maps_dict = {}

        for k in cond_keys:
            cond_maps = [x[k] for x in data]
            cond_maps = torch.stack(cond_maps).to(dtype=self.weight_dtype, non_blocking=True)
            cond_maps_dict[k] = cond_maps
...
def main(args):
    ...
    if accelerator.is_local_main_process:
        print("dataset_option: ", args.dataset_option)
        print("is_random_spatial: ", args.is_random_spatial)
        print("is_random_temporal: ", args.is_random_temporal)
        print("is_random_shift: ", args.is_random_shift)
    ...
    if args.cond_option == "full":
        NUM_INPUTS = 3 
    elif "+" in args.cond_option:
        NUM_INPUTS = 2
    else:
        NUM_INPUTS = 1

    print("NUM_INPUTS: ", NUM_INPUTS)
    transformer = CogVideoXTransformerFlex.from_pretrained(
        args.pretrained_model_name_or_path,
    ...
    transformer_lora_config = LoraConfig(
        ...
        modules_to_save=["fuse_linear"]
    )
    transformer.add_adapter(transformer_lora_config)

    if args.pretrained_lora_path != "None":
        print("load from pretrained lora path")
        state_dict = load_file(args.pretrained_lora_path)

        for key in list(state_dict.keys()):
            mod_key = key[len("transformer."):]
            state_dict[mod_key] = state_dict.pop(key)
        
        set_peft_model_state_dict(transformer, state_dict)

    print("trainable layers:")   
    if args.train_fuse:      
        transformer.fuse_linear.modules_to_save["default"].weight.requires_grad = True
        transformer.fuse_linear.modules_to_save["default"].bias.requires_grad = True
    else:
        print("WARNING: not training the fuse layer")
        transformer.fuse_linear.modules_to_save["default"].weight.requires_grad = False
        transformer.fuse_linear.modules_to_save["default"].bias.requires_grad = False
    print(transformer.fuse_linear.modules_to_save["default"].weight.requires_grad)            
    print(transformer.fuse_linear.modules_to_save["default"].bias.requires_grad)   
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            CogVideoXFlexControlPipeline.save_lora_weights(
                output_dir,
    ...
    def load_model_hook(models, input_dir):
        ...
        if not accelerator.distributed_type == DistributedType.DEEPSPEED:
            ...
            transformer_ = CogVideoXTransformerFlex.from_pretrained(
                args.pretrained_model_name_or_path, subfolder="transformer", num_inputs=NUM_INPUTS
            )
            transformer_.add_adapter(transformer_lora_config)

        lora_state_dict = CogVideoXFlexControlPipeline.lora_state_dict(input_dir)
    ...
    transformer_parameters = [
        ...
        if p.requires_grad and "fuse" not in name
    ]
    linear_parameters = [
        p for name, p in transformer.named_parameters()
        if p.requires_grad and "fuse" in name
    ]
    ...
    linear_parameters_with_lr = {
        "params": linear_parameters,
        "lr": args.learning_rate,
    }
    params_to_optimize = [transformer_parameters_with_lr, linear_parameters_with_lr]
    ###### modified end #########
    ...
    dataset_init_kwargs = {
        ...
        "image_to_video": True,
        "seg_column": args.seg_column,
        "tracking_column": args.tracking_column,
        "cond_option": args.cond_option,
    ...
    train_dataset = VideoDatasetUniform(**dataset_init_kwargs)

    collate_fn_video = CollateFunctionFlexVideo(weight_dtype, args.load_tensors)
    ...
    accelerator.print(f"  LORA rank = {args.rank}")
    accelerator.print(f"  LORA alpha = {args.lora_alpha}")
    ...
    if accelerator.is_main_process:
        ...
        pipe = CogVideoXFlexControlPipeline.from_pretrained(
            args.pretrained_model_name_or_path,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                # prepare transformer control condition inputs
                cond_maps_dict = batch["cond_maps_dict"]    

                flag_zeros = {k: False for k in cond_maps_dict}
                if random.random() < args.cond_dropout:
                    flag_zeros["id"] = True

                multi_cond_states = [
                    CogVideoXFlexControlPipeline.cond_to_latents(cond_maps_dict[k], vae, video_latents.shape, flag_zero_image=flag_zeros[k])
                ...
                model_output = transformer(
                    ...
                    multi_cond_states=multi_cond_states,
                    timestep=timesteps,
                ...
                # add_loss = torch.mean(
                #     (weights * (model_pred[:,-1] - target[:, -1]) ** 2).reshape(batch_size, -1),
                #     dim=1,
                # ).mean()
                # loss += add_loss
                        ...
                        if accelerator.is_main_process and args.checkpoints_total_limit is not None:
                            ...
                            if len(checkpoints) >= args.checkpoints_total_limit:
                                ...
                                for removing_checkpoint in removing_checkpoints:
                                    ...
                                    rmdir_except_lora(removing_checkpoint)
